...
- **PR C (next)** — walk-forward evaluator on top of `run_backtest`: train on N months, test on next M months, roll forward. Persist per-window `BacktestSummary` to disk.
- **PR D (optional)** — parameter-search grid harness.
